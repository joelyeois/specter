...
**Three per-step costs in a reconstruction are bookkeeping, not physics, and were removed on 2026-09-03** (`tests/test_reconstruct_perf_ports.py` pins each against the old spelling): (1) `Scattering.rytov`/`firstborn`/`kinematic` accumulate their slice sum a z-chunk at a time (`_fourier_slice_sum`), with the propagator cached complex and flipped ONCE for the negative Ewald sign instead of flipping the volume, and the amplitude-contrast and `i*sigma*dz` scalars applied to the (B, Y, X) sum instead of to complex copies of the volume. The chunks MUST come from `V.split`, never `V[:, z0:z1]`: a narrow slice's backward zero-fills a whole gradient volume per chunk (the O(Z^2) trap 41e0125 took out of multislice), 17 ms each at a 512 box. (2) The `on_train_batch_end` k-mask hook is `irfftn(rfftn(V) * kmask_half)` on an ifftshifted half-spectrum mask cached per tensor and device; the centred `fft3(..., shift=True)` round trip it replaced cost 137 ms and 6.5 GiB per step, a third of the step. (3) The per-step loss logs keep device tensors, the progress bar refreshes every ten steps, and `potential_scale`'s unity check is a host bool. Together: box 512, batch 3, epoch 61 -> 33 s, peak 27.7 -> 22.2 GiB. A fourth change, `VolumeRotator.rotate_real` recomputing each image's sampling grid in the backward under `torch.utils.checkpoint` when the volume requires grad, halves the peak again (12.3 GiB) for 24% more time at batch 3 and pays back at batch 6; simulators run forward-only and never take that path. Fused AdamW was measured 8 ms/step faster and rejected because Lightning's 16-mixed plugin refuses the grads it produces.
...
- **Two host-side facts shape `specter simulate micrograph`'s cost, and neither is a physics choice.** (1) PyTorch sizes its CPU thread pool from the machine, and every CPU tensor op pays a fixed sync cost proportional to it; on a tensor of a few hundred elements that cost IS the op. On this 128-core host `coords.poisson_disk_neighbors_3d` took 49 s at the default pool and 0.24 s at 4 threads, for `torch.equal` coordinates, and the same overhead sat on the template potential, tile placement and particle insertion. `specter.cpu_threads.limited_cpu_threads` caps the pool around exactly those loops (both Poisson-disk samplers at 1, `insert_particles_into_micrograph` and `IceBank._place_tiles` at 8). It is deliberately NOT process-wide: the big CPU ops want the pool, and the CPU ice phase got slower at 16 threads. Do not "simplify" it into one `torch.set_num_threads` at startup. The cap is not free either: every `torch.set_num_threads` that resizes the 128-thread pool costs ~12 ms, so a cap entered *inside* a loop multiplies that by the loop count -- `insert_particles_into_micrograph`'s per-call cap, reached once per chunk from `CrowdWithDuplicates.forward`, cost a 64-particle `simulate particles` at 128 px 1812 resizes and 21 s of a 28 s run. `process_volume` now caps once per batch around the crowd loop, so the inner caps find the pool already small and return without touching it (`tests/test_forward_perf_ports.py::test_process_volume_caps_cpu_threads_once_per_batch`). When profiling, look for `set_num_threads` in `tottime` first. (2) The specimen canvas lives on the host (33.5 GB at 4096 px), and `blend_ice_into_volume` used to build the ice on `V.device`, which put tile placement, splat, kernel convolution and occupancy blur on the CPU and cost 11 of a 2048-px micrograph's 13 minutes. An IceBank blend into a host volume now runs slab by slab on the bank's device (`_blend_ice_slabwise`, via `IceBank.big_ice_positions`, which draws every molecule once and parks them sorted by z on the host); same result to 1e-7 relative, checked in `tests/test_micrograph_perf_ports.py`. 2048 px: 6m57s / 43.5 GB -> 34 s / 17.8 GB; 4096 px: 1h19m / 144 GB -> ~1-3 min / 47 GB, and it runs under a 12 GiB device cap (molecules are bucketed on the HOST, never sorted on the device). Multislice was 1 s of that all along.
...
- **`specter match particles` (`pipelines/_match.py`, package `match/`) derives a `ParticleStackConfig` that matches a real particle set** and writes it as `matched.toml` plus a Markdown/PNG report, without CryoSPARC anywhere in the loop. It drives the ordinary `run_particle_stack` for every probe, so the config it writes is exercised by exactly the code that will run it. Order of operations is load-bearing: (1) a pose-alignment check (noiseless render at the refinement's poses correlated pair-by-pair with the experimental images against a shuffled pairing) that stops the run on failure, because the 2026-09-01/02 EMPIAR-11377 sweep spent 60 classifications on poses rotated 142 degrees from the experiment and no rotationally averaged statistic could see it; (2) detector MTF/DQE(0), coincidence radius and dose envelope from the acquisition card only (`detector_model`, `dose`, `dose_rate`, `energy_filter` -- the four numbers no particle file carries), never fitted; (3) ice thickness and neighbour spacing from small probe stacks scored on the background variance outside the particle; (4) a two-seed comparison at matched poses whose headline is the per-band **matched-pose SNR ratio** (`match/_metrics.py::matched_pose_snr`), the one statistic that separated datasets that mixed in CryoSPARC from those that did not. The report says plainly when a residual remains that no forward-model parameter can close; on every dataset tried it tracked the absence of an energy filter. Do not add a B-factor, potential-scale or coincidence-radius *fit* to this pipeline: each was, in earlier hand tuning, a stand-in for missing physics, and the residual belongs in the report, not in a knob. The coincidence radius comes from `detectors.py`'s `EXCLUSION_RADIUS_PX` (physical pixels; Falcon 4i 2.0 px *with* DQE(0)=0.92 modelled, K2 2.54) and `HARDWARE_FRAME_RATE_HZ`, converted by `coincidence_radius_for_simulation` at constant occupancy per exclusion cell per frame -- it is a property of the camera unit, a prior for other units of the same model, and is treated as such.
...
- **Potential kernels are odd-sized and voxel-centred, and the ice canvas is convolved periodically.** `compute_supersampling_parameters` keeps the super-sampled grid EVEN (no sample at the 1/r origin, which is what made Kirkland blow up on an odd fine grid) and the pooled kernel ODD, doubling an odd super-sampling factor so both hold. Until 2026-09-02 the pooled size fell out of the fine-grid rounding and was even for 20 of 36 pixel sizes (0.731 A gave 8^3, 1.5 A gave 4^3; 1.0 A happened to give 5^3), and an even kernel has no centre voxel, so every `method="3d"` potential landed half a voxel off its coordinates. On the ice canvas that combined with a linear 'same' convolution to leave the low face plane at HALF its potential and the high face at 92%; solvate's reflect padding then mirrored that into a dark line at the box boundary on every slice, which the CTF turned into a rim and ring scaling with ice thickness (the artifact was in every simulated particle, and grew when the hydrogen shell widened the kernel). `potential_from_deltas(boundary="periodic")` wraps instead, used by both `IceBank` canvas builders: a canvas is a piece of bulk, not a molecule in a box, and within a kernel radius of a face the wrapped neighbours have the status of an unrelaxed tile seam. Proteins keep `"linear"`. `tests/test_ice_canvas_faces.py` pins face-plane uniformity, the centroid, and the grid parity rules; do not "simplify" the parity rules back to a plain even/odd choice.
...
- **How much ice a voxel gets is decided by how full that voxel already is, and the potential is asked that question at the WATER's length scale, not the nucleus's.** `potential/_occupancy.py`'s `potential_occupancy(V, pixel_size)` coarse-grains `V` over `WATER_COARSE_GRAIN_SIGMA_A = 2.0` A and returns `clamp(blur(V)/FULL_OCCUPANCY_POTENTIAL_V, 0, 1)`; `blend_ice_into_volume` scales the ice field by `1 - occupancy`. **Sigma is in ANGSTROM, and that is the whole point** -- a per-voxel width would put the grid back into the answer. Two rules preceded it, both wrong, and neither should return. `V < 0.05 * V.max()` (until 2026-08-31) was relative to the volume's own contents, so adding gold fiducials to a 1000x4000x4000 A specimen took `V.max()` from 17.85 to 35.18 V and the fraction of occupied voxels receiving ice ON TOP of themselves from 12.9% to 28.8%; it altered ice on 13.87% of all OTHER voxels when one synthetic gold blob was added and nothing else changed. Unblurred `clamp(1 - V/7)` (until 2026-09-01, spelled `ice_occupancy_weight`, now DELETED) fixed that and left a worse one: a potential is cusped, so it finds the boundary of every ATOM rather than of the MOLECULE and pours bulk ice into the gaps between BONDED atoms 1.5 A apart. Measured on 1A6M at 1 A/voxel it admitted 0.597 of full-strength ice into voxels solidly inside the protein, and the excluded volume it recovered ran 0.36 of the molecule's own volume at 0.75 A/voxel to 0.87 at 4 A -- a 144% spread, against 0.9% blurred. The blur is not an extra approximation: a coarse voxel's own average has already removed the cusps, so raw and blurred agree exactly beyond ~8 A and the convolution is SKIPPED below a quarter voxel (at 10 A/voxel it is a no-op, correctly). It reads past whatever slab it is given, so `blend_ice_into_volume` widens each z-chunk by `occupancy_blur_halo_voxels` and discards the margin; without that every chunk boundary prints into the ice as a seam. `FULL_OCCUPANCY_POTENTIAL_V = 7.0` is protein's mean inner potential (6.81/7.00/7.07/6.84 V across 1A6M/1FA2/7VD8/6QZP); it lives in `potential/_occupancy.py` beside its provenance and two open caveats and is re-exported from `specter.ice`, so do not add a second copy -- it rests on the standard partial specific volume 0.73 cm3/g, a thermodynamic rather than geometric volume, and unlike the ice value it has no external citation. Gold and carbon sit far above it, clamp to zero and exclude water outright; a partial-volume voxel at a particle's edge takes partial ice, which is what belongs there. One reference serves every material because the blend sees a single summed volume: material lighter than protein is under-excluded (a bilayer's acyl core keeps ~23% of its water), which is bounded and one-directional.
- **A geometric occupancy field, built from van der Waals radii, was tried and REMOVED on 2026-09-01** (recover it from 8599e49). It is sharper -- 0.006 of full ice inside a molecule against the blur's 0.211 -- but it changes nothing any specter output can see, because a Gaussian blur CONSERVES the integral of V and therefore the total displaced water; it only moves where the displacement sits, taking too little from the interior and putting the same amount into a halo just outside. Measured on a ribosome column: 2170.8 V*A of ice under the blur against 2171.3 under geometry, 0.02% apart. Every shipped artifact either integrates along a ray (single-particle images, and tilt series -- the blur is isotropic, so conservation holds at any tilt) or carries no ice at all (`specter build tomogram` writes a clean volume plus labels; ice enters only at `simulate tiltseries`). It also cost more than it looked: combining it with the blurred field by `max` OVER-excluded by 2.2%, since that keeps the blur's halo artifact and the geometric interior at once. Do not reintroduce it unless a ground-truth volume ships WITH ice in it, which is the one case where the redistribution stops cancelling.
