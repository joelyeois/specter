...
- That command is also the only one with two output layouts. By default it writes `output_dir/run_name/`, the same cwd-relative rule as everything else. Setting `project` instead routes the run through `specter.jobs`: the directory becomes `job_base_dir/project/J00N/`, a `job.json` records every parameter plus the git commit, and pinning `job_id` resumes into an existing directory (this is how a manual `halfset="A"` then `halfset="B"` pair shares one, with `Job.create` asserting every *other* setting matches). `job_base_dir` defaults to `output_dir`, so opting in doesn't leave `specter-data/`. `nps_weight` (a per-frequency tensor) and `lpp_params` (a nested table) are the two `Ghostbuster` arguments with no config field, since neither has a spelling a flag can carry.
