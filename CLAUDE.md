...
- **Where a generator knows its own geometry, occupancy comes from that instead, and beats the blur on sharpness.** `potential/_occupancy.py`'s `atomic_occupancy` rasterizes the union of van der Waals balls (radii x `SOLVENT_EXCLUDED_RADIUS_SCALE = 1.30`, MEASURED not derived -- a union of overlapping balls does not grow as `r**3`, so the `(1/0.685)**(1/3)` that argument gives lands at 0.85 of target). It admits 0.006 of full ice inside a molecule against the blur's 0.211, and takes no potential as input, so `potential_scale`, the parameterization and a B-factor cannot move it. `run_particle_stack` builds it and `ImageGenerator` carries it as a buffer rotated by the same affine as `V`; `process_volume` combines it with the blurred field by `max`, BEFORE `potential_scale` is applied and AFTER crowding is summed in. Both orderings are load-bearing: reading occupancy off a scaled `V` would let a contrast knob change how much water the specimen displaces, and a caller-supplied field describes the target particle only, so crowding duplicates would otherwise take ice at full strength through their middles. It is NOT strictly better than the blur -- it is 12-13% voxel-size dependent where the blur is 0.9% (the radius inflation creates concave seams a one-voxel ramp erodes), and 60x more expensive to extend to crowders. What it buys is edge sharpness and freedom from the single-material constant.
