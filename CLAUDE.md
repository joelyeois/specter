...
- **The bilayer's `psi(z)` is a plane average, and has to be calibrated against one.** `membrane/_profile.py`'s profile is looked up by signed distance, so its amplitude is the mean potential over a plane at height z, not the value at an atom's centre. Calibrating against an isolated atom's peak (which specter did from 2026-08 until 2026-08-30) overstated the bilayer 5.1x, and no `voxel_size` fixes it: a cusp has no grid-independent value -- phosphorus reads 400.5 V at 0.5 A, 26.9 at 2.0, 4.1 at 4.0. The rationale on record, that a plane average dilutes the peak ~20x, measured the FULL reference patch; `compute_bilayer_profile`'s `lateral_core_fraction=0.6` window excludes the under-populated jittered edges and recovers 64.9 A^2 per lipid against the patch's own 65.0 target. `CALIBRATION_VOXEL_SIZE_A` is a fixed fine grid on purpose, decoupled from the render voxel size; resolution loss is applied once, later, by `_raster.py`'s anti-aliasing.
