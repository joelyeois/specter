...
docs-figures/                  # Tracked scripts that regenerate docs/assets/images/ figures for Concepts pages —
                              ...
                              # the third kind: it walks the live `click` objects and writes docs-includes/
                              # cli-reference.md, the command tables docs/api/cli.md includes.
docs-includes/                 # Generated Markdown fragments that docs/ pulls in via pymdownx.snippets (base_path is
                              ...
                              # cli-reference.md is regenerated by `python docs-figures/cli_reference.py`, and
                              # tests/test_cli_reference_docs.py fails when the committed copy no longer matches the
                              # CLI, so a new flag surfaces as a test failure rather than as a stale docs page.
