...
- **How much ice a voxel gets is decided by how full that voxel already is, against an ABSOLUTE reference -- never a fraction of the volume's own maximum.** `ice/_bank.py`'s `ice_occupancy_weight` returns `clamp(1 - V/FULL_OCCUPANCY_POTENTIAL_V, 0, 1)`, the share of the voxel still available to water, and `blend_ice_into_volume` scales the ice field by it. Until 2026-08-31 the rule was `V < 0.05 * V.max()`, which is relative to whatever else is in the volume, so an unrelated component moved it: measured on a 1000x4000x4000 A specimen, adding gold fiducials took `V.max()` from 17.85 to 35.18 V and the fraction of occupied voxels receiving ice ON TOP of themselves from 12.9% to 28.8% -- one config, two contrast regimes, decided by whether a bead happened to be present. Isolated on a single specimen with one synthetic gold blob added and nothing else changed, the old rule altered ice on 13.87% of all OTHER voxels and the new rule on 0.00%. It was also binary, so a voxel just under the cut took ice at full strength and its neighbour just over took none. Do not reintroduce a threshold keyed to `V.max()`, a percentile, or anything else the volume's contents can move. `FULL_OCCUPANCY_POTENTIAL_V = 7.0` is protein's mean inner potential (6.81/7.00/7.07/6.84 V measured across 1A6M/1FA2/7VD8/6QZP), and its own provenance and two open caveats are written at the constant -- it rests on the standard partial specific volume 0.73 cm3/g, which is a thermodynamic rather than geometric volume, and unlike the ice value it has no external citation. Gold and carbon sit far above it, clamp to zero and exclude water outright; a partial-volume voxel at a particle's edge takes partial ice, which is what belongs there. One reference serves every material because the blend sees a single summed volume: material lighter than protein is under-excluded (a bilayer's acyl core keeps ~23% of its water), which is bounded and one-directional.
