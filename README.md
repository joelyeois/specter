...
Parameters are loaded from a TOML config file (`configs/particle_stack/particle.toml` by default); any flag below overrides a single field without editing the file.

```bash
python demo-scripts/generate_particle_stack.py \
    --config configs/particle_stack/particle.toml \
    --pdb_code 6bdf \
    --n_particles 1000 \
    --num_pixels 256 \
    --pixel_size 1.056 \
