...
| Flag | Type | Default | Description |
| --- | --- | --- | --- |
| `--absorption_model` | `alpha` \| `inelastic_mfp` | `inelastic_mfp` | Where the imaginary potential comes from, for every probe and for the matched config. 'inelastic_mfp' derives it per material from a measured mean free path and ignores the dataset's amplitude contrast, which CTF estimation takes as an input and never fits. |
