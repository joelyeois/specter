...
seed doesn't reintroduce a competing hump between the leaflets. Two such
bugs were found and fixed by direct inspection before the approach was
retired. Because \(\sigma\) is far smaller than the leaflet separation,
the two-Gaussian construction cannot do that — by construction, not by
tuning.
...
An earlier version instead took the *laterally averaged* peak of the full
rendered lipid patch. Measured directly, that diluted the true single-atom
peak by ~20× (97.5 → 4.8 V), with no physical justification: a real
bilayer's phosphate atoms don't scatter more weakly than the same atom in
a protein. It was a real bug — a user-reported visual mismatch, a
transmembrane protein looking 5–10× brighter than the membrane around it,
led directly to finding it — not a design choice.

The general principle is worth stating, because it recurs: **the smoothness
real cryo-ET membranes show comes from the microscope's resolution limits,
applied to the ground truth after it is built** (CTF, multislice, detector
MTF — see [Forward simulation](../forward-simulation.md)), not from
pre-averaging the ground truth itself.
...
applying it as a post-hoc multiply on the rendered volume — which keeps
