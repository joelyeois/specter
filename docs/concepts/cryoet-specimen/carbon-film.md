...
analytic boundary — which is what gives it a rim that differs from
...
At the shipped default of 60 Å (CTS's own `JITTER_MAX`) the boundary's
radial standard deviation comes out to roughly a third of that, ~20 Å.
Note that even `edge_roughness = 0` is not a clean circle: the seeds are a
Poisson cloud, so the alpha shape is already ragged at the seed spacing.
...
This is the specific thing an earlier (deleted) analytic-boundary
implementation could not do. That version jittered a circle's radius per
angle and interpolated — which meant flat top and bottom faces, a
spatially constant density, and a rim that was a pure function of angle, so
every z-slice shared the exact same outline. Its own motivation was
sound (a fixed-point-count cloud degrades to sparse speckle as the volume
grows), and this implementation addresses that directly: the point count
scales with a physical seed density, so it does not thin out at larger
volumes or finer grids.
...
Hole *placement* is the one part deliberately not carried over from CTS,
which used a semi-random offset formula its own source flagged as needing
better randomisation. `edge_hole_center` is deterministic and
controllable instead.
...
lateral pad, and the 0.7× placed-density fraction — all either measured
