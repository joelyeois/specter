...
!!! info "Source"
    ...
    stamped by `TomogramSpecimenGenerator._stamp_microtubules`. Figures come
    from `docs-figures/cryoet_specimen_microtubules.py`, which calls the same
    functions the real code path does and measures the annotated numbers off
    the rendered volume.
...
\(R\) is the radius of the protofilament centres, \(a_\mathrm{lat}\) the
lateral spacing between protofilaments, \(r\) the monomer rise and \(s\) the
...
against the two deposited reconstructions its constants were measured from.
...
The last row is the one that matters. The stagger is **not** a fixed
...
microtubule frame, with protofilaments running as columns of monomers at
constant \((x, y)\), and their asymmetric unit is a small multi-protofilament
patch. `extract_mt_dimer` fits the tube axis from those neighbouring
protofilaments, lifts out one αβ dimer, and re-expresses it with \(+Z\) along
the protofilament (α → β, the plus-end direction) and \(+X\) pointing
outwards. It caches the result next to the downloaded structure.
...
not shear apart around a bend. Its flex angle is **derived**, not tuned:
matching the walk's \(\langle\theta^2\rangle = f^2/3\) to a worm-like chain's
\(2\Delta s / L_p\) gives
...
at tubulin's persistence length. A microtubule is stiff enough that this is
nearly straight: it wanders about half its own diameter over a whole
tomogram field.
...
inner wall, no protofilament-number transition. At a 10 µm radius of
curvature the real wall strain is \(r/R_\mathrm{curv} = 0.125\%\), far below
what a voxel grid resolves.
...
Every microtubule is centred on a uniformly random point and, by default,
spans the volume's diagonal, so it crosses the field the way real ones do
instead of trailing off from an interior point.

`confine_to_slab` (default on) rejects initial directions that would leave
the volume's thinnest dimension before the tube reaches full length. A
micrometre-long, 25 nm-wide tube cannot be steeply tilted inside 100–300 nm
of ice; rejecting on the actual box keeps that a consequence of the
specimen rather than a tilt-angle parameter.
...
microtubule is one object in the segmentation rather than ~950 loose dimers.
`export_picks` writes one `-microtubule` entry per tube carrying its axis
polyline, radius and protofilament count; the per-dimer copies remain
available in `microtubule_dimer_instances`.
