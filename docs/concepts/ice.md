...
Amorphous ice is not placed as water molecules dropped at random to the
right bulk density. `GradientSKIcemaker` instead optimises molecule
*positions* so that the resulting volume reproduces a measured structure
factor and a physically motivated interatomic energy, then `IceBank`
caches the result for near-instant reuse at simulation time. This page
covers the physics behind that optimisation; for the practical
cache/sampling side (choosing `ice_model`, building a replacement cache
at a different pixel size), see
...
Amorphous ice's structure is characterised by its structure factor
\(S(k)\), the radially averaged Fourier amplitude of the oxygen density.
...
published dataset. By default the target is recomputed natively at each
`GradientSKIcemaker` instance's own grid spacing and box size
(`compute_native_target`) rather than interpolated from a single
...
stuck at an S(k) loss of \(O(1)\)–\(O(10^3)\); a natively computed target
at the same spacing converges to \(O(10^{-5})\)–\(O(10^{-7})\) across
voxel sizes from 0.5 to 2.0 Å and box sizes from 16 Å up to 256 Å, beyond
the roughly 127 Å extent of the underlying MD cell, via safe
extrapolation in `compute_native_target`.

## The optimisation

Positions start uniformly random and are optimised by L-BFGS (the
...
and unrealistic local geometry, and was validated as the better balance
of energy-match quality against \(S(k)\) fidelity across voxel sizes from
0.5 to 2.0 Å and box sizes up to 256 Å.
...
energy to that value rather than minimising it unboundedly. Minimising
energy without a target drifts toward an arbitrarily low-energy, more
crystalline packing rather than the disordered target phase; matching a
measured reference value keeps the result in the right structural regime
instead. Set `mlbop_target=None` to fall back to unbounded minimisation.
...
- **The target is one phase of ice at one thermodynamic state.**
  ...
  amorphous, crystalline) needs a different MD reference frame, which is
