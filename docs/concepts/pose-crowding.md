...
drawn once per instance and stored as `(B, 4)` and `(B, 2)` tensors on
`ImageGenerator`/`ImageGeneratorFromCoordinates`. The representation, sign
conventions, and the real-space/Fourier-space application choice are
shared with every other pipeline in SPECTER and are described once, in
[Conventions](conventions.md#poses); this page does not repeat them.

What is specific to the two particle-generator classes is *where* the
pose acts. `ImageGenerator` rotates the pre-built volume itself.
`ImageGeneratorFromCoordinates` instead rotates the atomic coordinates
before voxelization, via the inverse rotation matrix
(`roma.unitquat_to_rotmat(Q).transpose(-2, -1)`) and an Å-space
translation applied directly to the coordinates, not through the
`[-1, 1]`-normalized grid used by `grid_sample`. Both reach the same
convention; see
...
- **Z-padding** extends the volume to accommodate the requested ice
  ...
  pixel_size`, zero-padded symmetrically to reach it.
...
Both are applied by `pad_volume` after pose but before crowding, scaling,
and ice, so every later stage in the chain sees the final, padded extent.
...
and is a different mechanism from `MicrographGenerator`'s field-of-view
assembly, which places independent, generally different particle species
across a whole micrograph; see
[Pipeline overview](pipeline-overview.md#how-the-generator-classes-use-it).

Sampling density is controlled by `crowd_min_distance` (the Poisson-disk
minimum separation) and bounded by `crowd_max_distance_z`/an implicit XY
bound derived from the volume size; `n_points=inf` by default, so
sampling continues until the box is full rather than stopping at a fixed
count. `crowd_chunk_size` limits how many duplicate volumes are rotated
per batch, trading GPU memory for speed; the default of 1 is the
memory-safe choice, and `None` rotates every duplicate at once.
...
candidate positions are kept or rejected by sampling against this profile
...
surface, which sits outside the ice entirely in the thinnest regions.
...
gates ordinary 3D placement regardless of `water_air_interface`: any
candidate whose particle radius would poke through its local surface is
rejected outright, so a placement never lands partly in vacuum.
