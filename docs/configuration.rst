...
.. literalinclude:: ../configs/particle.toml
   :language: toml
...
.. list-table::
   ...
   * - Convention
     - What it means
   * - A commented-out field
     - An optional feature. Uncommenting ``dose_max`` switches dose from
       fixed to randomised per particle; uncommenting ``cc`` enables the
       temporal-coherence envelope.
   * - ``_min`` without ``_max``
     - A fixed value for the whole run.
   * - ``_min`` and ``_max``
     - Sampled uniformly per particle, and recorded per particle in the
       output STAR file.
   * - Booleans
     - Written as values, not bare flags: ``--assembly True``.
   * - An unrecognised key
     - Raises an error rather than being ignored, so typos surface
       immediately.
