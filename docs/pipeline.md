...
- **Input**: `coordinates` (N,3) in Å, centred on the origin; `atomic_numbers` (N,)
...
interesting parts of the package. See [Ice](ice.md).
...
| Mode | Idea | Use for |
|---|---|---|
| `multislice` | Propagate slice by slice through the volume, accumulating multiple scattering. | Generation. The default and the most accurate. |
| `rytov` | Higher-order phase approximation without per-slice propagation. | Reconstruction, where the model runs thousands of times. |
| `firstborn` | Single-scattering approximation. | Fast comparisons. |
| `kinematic` | Variant of first Born. | Rarely. |
| `projection` | Sum the potential along Z, no Fresnel propagation. | Thin specimens, quick tests. |
| `ctf` | Returns a real projected potential rather than a wave. | Conventional CTF-only workflows. |
...
slice potential $V(z)$, and the Fresnel propagator $F(z)$
(complex exponential of $k^2$, wavelength and Δz) from slice
$z$ to the exit plane, the exit wave $\psi$ for each mode is:
...
where $\mathcal{F}$ denotes the 2D Fourier transform. `kinematic`
keeps the per-slice amplitude $e^{i\sigma\Delta z\,V} - 1$ exact,
where `firstborn` linearises it to $i\sigma\Delta z\,V$ — the two
agree when $\sigma\Delta z\,V \ll 1$ per slice and diverge for thick
or dense slices.
...
The transfer function is $\exp(-i\chi(k))$, where $\chi(k)$
sums whichever of these terms are supplied ($k$ = spatial frequency
magnitude, $\theta$ = its azimuthal angle, $\lambda$ =
wavelength):
...
uniform $\chi_{phaseshift} = -\phi_0$, forced to zero at DC to keep
...
where $\alpha_c$ is the beam convergence semi-angle
(`convergence_angle`) and $\Delta f_c$ is the chromatic focus
...
| Envelope | Enabled by | Represents |
|---|---|---|
| B-factor | `bfactor` | Generic resolution falloff |
| Spatial coherence | `convergence_angle` | Finite source size |
| Temporal coherence | `cc` | Energy spread, lens and voltage instability |
| Dose | `dose_envelope` | Cumulative radiation damage (Grant & Grigorieff 2015) |
...
!!! important
    **Sign convention.** SPECTER's raw output is physical: dense regions
    ...
    [Reconstruction](reconstruction.md). Micrographs and tilt series are standardised but
