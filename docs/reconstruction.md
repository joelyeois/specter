...
| Argument | Controls | If unset | If given a value | Status |
|---|---|---|---|---|
| `lr` | The 3D volume | frozen | refined + scheduled | Verified — the primary, tested path |
| `lr_R` | Per-particle orientation | poses held fixed | refined, constant rate | **Unverified** — wired in, not yet checked for correctness |
| `lr_T` | Per-particle shift | shifts held fixed | refined, constant rate | **Unverified** — wired in, not yet checked for correctness |
| `lr_D` | One global defocus offset | defocus held fixed | refined, constant rate | **Unverified** — wired in, not yet checked for correctness |
...
| Set this | Uses | Choose it when |
|---|---|---|
| `use_ncc=True` | Normalised cross-correlation | Image scale or gain does not match the model's absolute prediction. |
| `learn_noise_model=True` | Learned per-shell noise weighting | You want RELION-style weighting, estimated from the residuals as training proceeds. |
| `nps_weight=...` | Fixed noise-power weighting | You already know the noise spectrum and want it held constant. |
| none of the above | Mean squared error | The default. Start here. |
...
where $p$ is the simulated image, $t$ the experimental one,
...
!!! important
    **Intensity convention — the single most common setup error.**
    ...
    where $p$ is the raw normalised particle stack. The
...
resolution. For two volumes' Fourier transforms $F_1(\mathbf{k})$,
$F_2(\mathbf{k})$, FSC is the normalised cross-correlation within
each spatial-frequency shell $|\mathbf{k}|$:
...
| Function | Compares | Criterion |
|---|---|---|
| `plot_map_to_model_fsc` | A volume against a known reference | FSC = 0.5 |
| `plot_halfmap_fsc` | Two independent half-maps | FSC = 0.143 |
