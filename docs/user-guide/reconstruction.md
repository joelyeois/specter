...
uses reproduces a stack of experimental images. Where the forward pipeline
goes coordinates &rarr; images, `specter reconstruct particle` goes
images &rarr; volume. See [Pipeline overview](../concepts/pipeline-overview.md)
...
particle` -- the two names are the same command, so the tool answers to
whichever one you or the docs use.
...
and fill in the two paths reconstruction actually reads from:
...
statistics the loss is weighted by, so it must match the dataset's real
fluence, not a placeholder. Reconstruction currently reads CryoSPARC `.cs`
only -- there is no RELION `.star` equivalent yet, unlike the [CryoSPARC
dataset twin](dataset-twin.md) path on the forward side (see [Generate a
particle stack](particle-stack.md#example-matching-empiar-11377) for a
worked `.cs`-driven example).
...
resulting half-maps -- the same independent-validation idea CryoSPARC and
RELION use to report resolution without a model bias. `--halfset all`
...
reference on every epoch, for reporting only -- never optimised against.
`--cryosparc_ref` accepts a comma-separated `"<A>,<B>"` pair as well as a
single path. The pair matters specifically for a gold-standard run: it
reconstructs both halves from one config, so without it both halves would
be plotted against the same CryoSPARC half-map, silently mislabelling one
of the two overlays. A single path is fine for `--halfset A`/`B`/`all`,
where only one half (or the whole stack) is ever being compared.
...
- `epochs/<NNN>_A.mrc` / `epochs/<NNN>_B.mrc` -- per-epoch snapshots of each
  half.
- `fsc_gold_standard.png` -- the final half-map FSC, computed once both
  halves finish.
...
- `epochs/fsc_<NNN>_A.png` / `epochs/fsc_<NNN>_B.png` -- per-epoch
  map-to-model FSC against `--fsc_ref`, if one was given.
...
Four resolution numbers are recorded per epoch in `job.json`: map-to-model
and half-map, each masked and unmasked. Map-to-model needs `--fsc_ref` to
exist at all; the masked entries additionally need `--fsc_mask` (and, for a
`--test_run`, are skipped if binning has left the mask's shape mismatched
against the volume). Each of the four degrades independently rather than
failing the run -- a config with no reference map simply has no
map-to-model entries, not a crash. The masked numbers come from a separate
computation, not read off the figures: the plotting helpers always report
the *unmasked* crossing even when the curve they draw is masked.
...
directory, which is useful for running each half on a different machine or
at a different time:
...
Every reconstruction run is tracked through `specter jobs`, the same way
every other `specter` pipeline is -- see [Manage jobs](jobs.md) for the
directory layout, `job.json` contents, and the `specter jobs`
list/show/diff CLI. `--project` and `--job_id` behave exactly as documented
there.
...
with gradients all-reduced every step. For a gold-standard run, a
...
volume every epoch, applied in real space or Fourier space via
