...
- **Tilt geometry** — `min_tilt_angle`/`max_tilt_angle` (degrees) and
  ...
  below — that needs the Python API, not this CLI.
...
- **Ice** — `ice_model`: `"gd"` (default, `IceBank`'s cached
  `GradientSKIcemaker` configs — realistic, effectively free at this cache
  size), `"random"` (cheap, low-fidelity `RandomIcemaker`), or `"none"`
...
result straight in as this run's volume — no path to copy by hand:
...
flags — those aren't reachable through `simulate tiltseries`); imaging
