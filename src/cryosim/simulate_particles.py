...
class CustomWriter(BasePredictionWriter):
    ...
    def write_on_epoch_end(self, trainer, pl_module, predictions, batch_indices):
        ...
        torch.save(images, os.path.join(self.output_dir, f"predictions_{trainer.global_rank}.pt"))

        # optionally, you can also save `batch_indices` to get the information about the data index
        # from your prediction data
        idx = torch.squeeze(torch.tensor(batch_indices)).reshape(-1)
        # idx = torch.concat(idx, dim=0)
        torch.save(idx, os.path.join(self.output_dir, f"batch_indices_{trainer.global_rank}.pt"))
...
def main():
    parser = argparse.ArgumentParser(description="Simulate particle images with multi-GPU Lightning.")
    parser.add_argument("--V_path", type=str, required=True, help="Path to the saved V tensor (.pt)")
    ...
    parser.add_argument("--output_path", type=str, default="../cache/images.pt", help="Path to save the final image stack")
