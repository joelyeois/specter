...
micrometers/mm/degrees -- ordinary physical units, just not Angstrom. Its
...
1/Angstrom -- ``torch_ctf._ctf_core._build_freq_grid`` calls
...
def zernike_rho_max(image_shape: tuple[int, int], pixel_size: float) -> float:
    """Maximum (corner) spatial frequency of an fftfreq grid, in 1/Angstrom.
    ...
    -------
    float
        The grid's maximum radial spatial frequency, in 1/Angstrom.
    """
    h, w = image_shape
