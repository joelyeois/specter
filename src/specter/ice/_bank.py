    ...
    def build(self, num_unique: int | None = None, **kwargs: Any) -> None:
        ...
        **kwargs
            ...
            - ``gd``: ``n_steps``, ``lr``, ``rep_strength``, ``mlbop_strength``,
              ``mlbop_target`` -- see :meth:`~specter.ice.GradientSKIcemaker.
              generate_ice` for their defaults (matching real amorphous ice
              via ML-BOP energy) and how to opt back into the old
              geometric-only ``rep_strength`` penalty.
            - ``ap``: ``reduce_fraction``
