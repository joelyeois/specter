...
class ImageGeneratorFromCoordinates(ParticleGeneratorBase):
    ...
    ice_model : str, optional
        ...
        (naive random placement). Ignored when ``icemaker`` is provided.
    ...
    icecube_size : int, optional
        Side length (in voxels) of each cubic ice block generated by the
        ``IceBank``. Smaller cubes are generated and optimised much faster
        (especially for the ``'gd'`` method) and are then tiled to fill the
        volume. Default 256. Ignored when ``icemaker`` is provided.
    icemaker : IceBank, optional
        A pre-built :class:`~specter.ice.IceBank` instance to reuse across
        multiple ``ImageGenerator`` instances. When supplied, ``ice_model``,
        ``num_unique_icecubes``, ``ice_build_batch_size``, and ``icecube_size``
        are all ignored. ``ice_thickness`` is still respected for computing
        ``nz``. The bank must already be built (i.e. :meth:`IceBank.build` has
        been called or will be triggered lazily on first use).
...
class ImageGenerator(ParticleGeneratorBase):
    ...
    icemaker : IceBank, optional
        A pre-built :class:`~specter.ice.IceBank` instance to reuse across
        multiple ``ImageGeneratorFromCoordinates`` instances. When supplied,
        ``ice_model``, ``num_unique_icecubes``, ``ice_build_batch_size``, and
        ``icecube_size`` are all ignored. ``ice_thickness`` is still respected
        for computing ``nz``. The bank must already be built (i.e.
        :meth:`IceBank.build` has been called or will be triggered lazily on
        first use).
    scattering_model : str, optional
