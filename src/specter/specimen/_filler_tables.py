"""
Filler reference tables consumed by `build_filler_pool_specs`
(`specter.specimen._cytosolic_filler`), kept apart from that logic since
they are plain transcribed data.
...
weighting (their normalization of proteomic abundance data). It is used
only when abundance weighting is requested, and then only for the *ratio*
between species -- see `build_filler_pool_specs`.
