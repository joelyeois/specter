"""
Reference list of macromolecular complexes from the CryoETSim simulated
cryo-ET dataset, for filling the "everything else" background of a
specimen (crowding below/around whatever specific species you're
annotating as targets) -- a second, independent source alongside
`cytosolic_filler.PEI2016_CROWDING_TABLE`, additive to it (see
`SpherePackingSpecimenGenerator`'s target/filler split; both tables are
adapted to its `filler_specs` shape via `build_filler_pool_specs` below).
...
def build_filler_pool_specs(
    ...
    entry -- both `CRYOETSIM_PARTICLE_TABLE` (this module) and
    `cytosolic_filler.PEI2016_CROWDING_TABLE` qualify, so the same helper
    adapts either (or both, called twice and concatenated) into one
    filler_specs list. `categories` only has an effect on tables that
    carry a ``"category"`` key (`CRYOETSIM_PARTICLE_TABLE`); entries
    without one are kept regardless of this filter.

    Parameters
    ----------
    table : list of dict
        e.g. `CRYOETSIM_PARTICLE_TABLE` or
        `cytosolic_filler.PEI2016_CROWDING_TABLE`.
