...
from ._cts_placement import ParticlePlacer, ParticleSpec, PlacementMode
...
def _build_membrane_location_fields(
    ...
    masks and a ``"membrane"`` outward-normal field, from the already-placed
    membrane instances' local geometry (``BlobMembraneInstance.shell_mask``/
    ``interior_mask``/``normal_field``, see ``_cts_membrane.py``), rotated
    and positioned exactly as each membrane's own density was.
    ...
    -------
    location_masks : dict
        Keys ``"membrane"``, ``"vesicle"``, ``"cytosol"`` -> bool tensors,
        shape `target_shape`. If no membranes were placed, `"membrane"`/
        `"vesicle"` are all-False and `"cytosol"` is all-True (equivalent
        to unrestricted "any" placement).
    normal_fields : dict
    ...
    """
    membrane_mask = torch.zeros(target_shape, dtype=torch.bool)
    ...
    for placed in placements:
        ...
        bounds = clip_insert_bounds(
            center.tolist(), rotated_shell.shape, membrane_mask.shape
        )
        if bounds is not None:
            dst, src = bounds
            membrane_mask[dst] |= rotated_shell[src]
    ...
    cytosol_mask = ~(membrane_mask | vesicle_mask)
    location_masks = {
    ...
    return location_masks, normal_fields
...
class CryoTomoSimSpecimenGenerator:
    ...
    def generate(self) -> torch.Tensor:
        ...
        protein_specs_for_placer: list[ParticleSpec] = []
        for spec in self.protein_specs:
            ...
            density = builder.forward(pdb.coordinates, method="3d")
            protein_templates[spec.pdb_source] = density
            protein_specs_for_placer.append(
                ParticleSpec(
        ...
        # Generate and place membranes before proteins, so membrane-flagged
        # protein locations can restrict to the actual placed membranes'
        # surface/interior (built below from ParticlePlacer's own placement
        # records, once membranes have landed in the volume).
        membrane_specs_for_placer: list[ParticleSpec] = []
        membrane_local_by_id: dict[str, BlobMembraneInstance] = {}
        if self.membrane_specs:
            mem_gen = MembraneBlobGenerator(v_size=self.v_size, seed=np_rng_seed)
            for i, mspec in enumerate(self.membrane_specs):
                for j in range(mspec.count):
                    ...
                    scaled_density = inst.density * reference
                    sid = f"membrane_{i}_{j}"
                    self.membrane_instances.append(inst)
                    membrane_local_by_id[sid] = inst
                    membrane_specs_for_placer.append(
                        ParticleSpec(
                            species_id=sid,
        ...
        placer.run(membrane_specs_for_placer)

        location_masks, normal_fields = _build_membrane_location_fields(
            placer.placements, membrane_local_by_id, self.target_shape
        )
        placer.run(
            protein_specs_for_placer,
