...
def get_rotation_matrices(sym: str, return_affine: bool = True) -> torch.Tensor:
    ...
    ``T``, ``O``, ``I1`` and ``I2`` are generated by closure from exact
    generators in RELION's settings (see `_matrices`); ``I`` is an alias of
    ``I2``. ``I1`` and ``I2`` are the two 222 settings, which differ in which
    yz-plane 5-fold axis lies closer to z.
